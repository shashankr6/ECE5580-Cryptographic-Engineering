...
void UARTGetPT(int len, char *rxbuffer){
    char c;
    int i = 0;
    while (i < len){
        if (UARTHasChar()){
            rxbuffer[i] = UARTGetChar();
            i++;
        }
    }
}

void UARTPutCT(int len, char *txbuffer){
    int i;
    for (i=0;i<len;i++){
        UARTPutChar(txbuffer[i]);
    }
}
...
//    uint8_t pt[32];// = "b9145a768b7dc489a096b546f43b231f";

    char *IV = "";
    uint8_t KeyAES[16] = { 0xca, 0xfe, 0xba, 0xbe, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x00, 0x00 };
    uint8_t PT[16];
...
//    }
    ...
    while (1){
        if (UARTHasChar()){
            GPIO_toggleOutputOnPin(GPIO_PORT_P2, GPIO_PIN0);
//            c = UARTGetChar();
            uartTrigger = UARTGetChar();
//            printf ("\n received char: %c", c);
//            printf("  count: %d", uart_char_count);
//            UARTPutChar(uartTrigger);
        }
